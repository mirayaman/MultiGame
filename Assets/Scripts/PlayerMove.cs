...
public class PlayerMove : MonoBehaviour
{
    ...
    [SerializeField, Range(1.0f, 10.0f)] private float JumpForce = 5.0f; //Сила толчка для прыэка
    ...
    private bool isGround() //Переменная для проверки наличия земли под ногами!
    {
        ...
        if(Physics.Raycast(ray, out hit, jumpDistance)) //Передаём данные из луча в переменную hit. Если есть столкновение и обьект находиться на дистанции меньшой или равной jumpDistance возврощаем true
        {
            return true;
        }
        return false;
    ...
    private void ControlePlayerCamera(float mouseX = 0.0f, float mouseY = 0.0f) //Метод управления камерой. Принимает позицию мышки.
    {
        ...
        camera.localEulerAngles = new Vector3(-rotationY, rotationX);//Меняем  положениу в пространстве
    }
    ...
    private void ControlePlayerMove(float horizontal = 0.0f, float vertical = 0.0f)//Метод перемещения игрока. Принимает в себя wasd или же стик джойстика
    {
        ...
        if(isGround() && Input.GetKey(KeyCode.Space))//Проверяем землю и нажатие клавиши прыжка!
        {
    ...
    private void Start() //В этом методе идёт поиск обьектов.
    {
		...
		if(camera == null)
        {
            camera = transform.Find("CameraBox"); camera = camera.Find("PlayerCamera"); //Это кастыль. Так делать плохо, но иногда можно.
        }
